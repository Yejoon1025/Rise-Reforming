...
// • All interactions/behaviors preserved (flip, keyboard, swipe, etc.)
// • Idle-swap / idle-sweep logic REMOVED
...
@keyframes cardflip-nudge-x {
  ...
  }, [active, displayItems, order, reversed, isIntersecting, flipped, dismissedFlipHint])
  ...
  useEffect(() => {
    ...
    const onWheel = e => {
      // discrete step wheel support (kept minimal, respects cooldown)
      if (wheelCooldownRef.current) return
      const dy = e.deltaY ?? 0
      const dx = e.deltaX ?? 0
      const mag = Math.abs(dy) > Math.abs(dx) ? dy : dx
      if (Math.abs(mag) < 2) return

      e.preventDefault()
      wheelCooldownRef.current = true
      const dir = (mag > 0 ? +1 : -1) * dirFactor
      go(dir)
  ...
  }, [dotLeft, reversed, isIntersecting])

  // Auto-reset flip state when the focused card changes.
// (Do NOT depend on displayItems here; it is re-created each render.)
useEffect(() => {
  setFlipped(prev => {
    if (!prev || Object.keys(prev).length === 0) return prev

    const originalIndex = order[active]
    const activeItem = items?.[originalIndex]
    if (originalIndex == null || !activeItem) return {}

    const activeKey = keyFor(originalIndex, activeItem)

    // Keep flip ONLY for the currently active card (if it was flipped)
    return prev[activeKey] ? { [activeKey]: true } : {}
  })
}, [active]) // intentionally ONLY active
    ...
    <section ref={sectionRef} className={`relative w-full ${className}`} aria-label="CardFlip Horizontal">
      ...
      >
        ...
        {displayItems.map((it, di) => {
          ...
          const appearInZone = isInSoftZone(di)
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  transform: `translate3d(0,0,0)${reversed ? " scaleX(-1)" : ""}`,
                  opacity: 1,
                ...
                >
                  ...
                  >
                    <div
                      ...
                      style={{
                        transformStyle: "preserve-3d",
                        transform: isFlipped && appearInZone ? "rotateY(180deg)" : "rotateY(0deg)",
                      }}
                    >
                      {/* front */}
                      <div
                      ...
                      >
                        ...
                        >
                          ...
                          <div
                            className={`w-full rounded-lg px-3 py-2 ${hasBackImage ? "mt-2 sm:mt-3" : ""}`}
                          >
                            <p
                              className={
                                hasBackImage
                                  ? "text-sm sm:text-base md:text-lg leading-relaxed text-[#e0e0e0]"
                        ...
                        {(it.linkedin || it.email) && (
                          <div className="absolute left-0 right-0 flex items-center justify-center gap-3" style={{ bottom: ICONS_HB }}>
                            {it.linkedin ? (
                              <a
                                href={it.linkedin}
                                target="_blank"
                  ...
                  {appearInZone && (
                    <div className="absolute inset-0 z-50 pointer-events-none">
                      {(() => {
                        ...
                        const edgePos = reversed ? "left-1 sm:left-1.5 md:left-2" : "right-1 sm:right-1.5 md:right-2"
                          ...
                          return (
                            ...
                            >
                              ...
                              <div
                                ...
                                } px-2 py-1 rounded bg-black/70 text-[#f8da9c] text-[10px] leading-tight whitespace-nowrap
